-- Add column to track stock effects on live_carts
ALTER TABLE live_carts ADD COLUMN IF NOT EXISTS stock_decremented_at TIMESTAMPTZ DEFAULT NULL;

-- Create function to apply paid effects for live carts (idempotent)
...
GRANT EXECUTE ON FUNCTION public.apply_live_cart_paid_effects(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.apply_live_cart_paid_effects(UUID) TO anon;