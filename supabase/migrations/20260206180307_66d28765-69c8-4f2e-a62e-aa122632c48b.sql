...
-- Fix handle_order_paid: v_item.item -> v_item.value
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND NEW.status = 'pago' AND OLD.status != 'pago' THEN
      ...
      LOOP
        ...
        IF v_current_stock IS NOT NULL THEN
          v_current_stock := jsonb_set(
            COALESCE(v_current_stock, '{}'::jsonb), ARRAY[v_size],
          ...
          v_current_committed := jsonb_set(
            v_current_committed, ARRAY[v_size],
    ...
    -- No reservation: direct from order_items
...
-- Fix revert_stock_on_cancel: v_item.item -> v_item.value
...
BEGIN
  ...
  UPDATE inventory_movements SET movement_type = 'reservation_reverted' WHERE id = v_movement.id;
