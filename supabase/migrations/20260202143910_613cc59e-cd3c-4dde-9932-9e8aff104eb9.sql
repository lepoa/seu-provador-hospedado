-- Drop and recreate with better JSONB handling
DROP FUNCTION IF EXISTS public.get_products_effective_prices(TEXT, UUID[]);
...
BEGIN
  ...
  WITH active_promotions AS (
    ...
    SELECT 
      ...
      -- Handle both string and array JSONB for category_discounts
      CASE 
        WHEN jsonb_typeof(pt.category_discounts) = 'array' THEN pt.category_discounts
        WHEN pt.category_discounts IS NOT NULL AND pt.category_discounts::TEXT != '' AND pt.category_discounts::TEXT != 'null' THEN
          CASE 
            WHEN jsonb_typeof(pt.category_discounts::JSONB) = 'array' THEN pt.category_discounts::JSONB
            ELSE '[]'::JSONB
          END
        ELSE '[]'::JSONB
      END AS category_discounts_parsed,
      -- Handle both string and array JSONB for product_discounts
      CASE 
        WHEN jsonb_typeof(pt.product_discounts) = 'array' THEN pt.product_discounts
        WHEN pt.product_discounts IS NOT NULL AND pt.product_discounts::TEXT != '' AND pt.product_discounts::TEXT != 'null' THEN
          CASE 
            WHEN jsonb_typeof(pt.product_discounts::JSONB) = 'array' THEN pt.product_discounts::JSONB
            ELSE '[]'::JSONB
          END
        ELSE '[]'::JSONB
      END AS product_discounts_parsed,
      pt.channel_scope
  ...
  product_level_discounts AS (
    ...
    CROSS JOIN LATERAL jsonb_array_elements(ap.product_discounts_parsed) AS elem
  ...
  category_level_discounts AS (
    ...
    FROM products_base pb
    CROSS JOIN active_promotions ap
    CROSS JOIN LATERAL jsonb_array_elements(ap.category_discounts_parsed) AS elem
